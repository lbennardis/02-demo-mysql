...
@Entity
@NamedQuery(name="Stazione.findNearest",query="SELECT indirizzi.nome,indirizzi.indirizzo,indirizzi.citta,indirizzi.latitudine,indirizzi.longitudine,3956 * 2 * (ASIN(SQRT(POWER(SIN((41.93236 - abs(latitudine)) * pi()/180/2),2)+ COS(41.93236 * pi()/180 )*COS(abs(latitudine) *  pi()/180)*POWER (SIN((12.52021 - longitudine)) *  pi()/180/2, 2)))) as distance FROM indirizzi having distance < 10 ORDER BY distance limit 10;")
